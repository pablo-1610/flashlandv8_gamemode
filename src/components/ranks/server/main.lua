...
_FlashLand.onReceiveWithoutNet("loaded", function()
    _FlashServer_Database.query("SELECT flash_ranks.id,flash_ranks.label,flash_ranks.weight,flash_ranks_permissions.permission FROM flash_ranks INNER JOIN flash_ranks_permissions ON flash_ranks.id = flash_ranks_permissions.rankId", {}, function(result)
        for row, data in pairs(result) do
            if (_FlashServer_Ranks.exists(data.id)) then
                ...
                _FlashLand.log(("Ajout d'un rang: ^2%s"):format(data.id))
                rank:addPermission(data.permission)
            end
        end
    end)
