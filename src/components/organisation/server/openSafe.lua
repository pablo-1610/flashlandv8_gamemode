...
---@param zone _Zone
_FlashServer_Organisation.openSafe = function(_src, player, zone, organisation)
    if (not (player)) then
    ...
    if (not (orgaGrade:hasPermission("orga.openSafe"))) then
    ...
    _FlashLand.toClient("organisation:openSafeMenu", _src, orgaData.jobName, orgaData.jobLabel, orgaData.inventory, orgaData.weapons)
end