...
_FlashLand.onReceive("orga:removePlayerWeapon", function(organisationName, weaponName)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("orga:removePlayerWeapon sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
    ...
    if (not (orgaGrade:hasPermission("orga.safeDepositWeapon"))) then
    ...
    _FlashServer_Loadout.player_removeWeapon(_src, weaponName, function(status)
        if (status) then
            orgaData:addWeapon(weaponName)
            orgaData:saveSafeLoadout()
            _FlashLand.toClient("orga:updateWeapons", _src, orgaData.weapons)
            player:sendSystemMessage(_FlashEnum_SYSTEMMESSAGE.SUCCESS, _Static_GenericMessages.PLAYER_DEPOSIT_WEAPON_SUCCESS)
        else
