...
_FlashLand.onReceiveWithoutNet("players:newLoadedPlayer", function(_src, data)
    ...
    ---@param player _Player
    local player = _Player(_src, data.flashId, data.identifier, data.rankId, json.decode(data.identity), data.cash, json.decode(data.skin), json.decode(data.outfits), data.selectedOutfit, json.decode(data.accessories), data.phone_number)
