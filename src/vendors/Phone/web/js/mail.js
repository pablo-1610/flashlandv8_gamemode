...
MI.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(MI.Phone.Data.PlayerData.charinfo.phone+"@carp.com");
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Gelen Kutun Boş!</p>');
        }
...
MI.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Sarı sayfalara atılan bir mesaj yok!</span></div>';
        $(".advert-list").append(element);
    }
}