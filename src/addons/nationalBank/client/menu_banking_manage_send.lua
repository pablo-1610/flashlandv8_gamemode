...
_FlashClient_NationalBank.drawer[5] = function()
    ---@type _BankAccount
    local account = _FlashClient_NationalBank.var.accounts[_FlashClient_NationalBank.var.selectedAccount]
        ...
        RageUI.Button(("~s~[~o~#%s~s~] %s%s~s~%s"):format(bankAccount.accountId, bankAccount.state == _FlashEnum_BANKACCOUNTSTATE.SUSPENDED and "~c~" or "~s~", bankAccount.label, (showPins and (" (~r~%s~s~)"):format(bankAccount.pin) or "")), ("Type: %s"):format(_FlashEnum_BANKACCOUNTTYPELABEL[bankAccount.type]), { RightLabel = _FlashUtils.math_price(bankAccount.balance) }, true, {
            onSelected = function()
                ...
                if (amount ~= nil and tonumber(amount) ~= nil) then
                    ...
                    _FlashLand.toServer("nationalBank:transfer", account.accountId, bankAccount.accountId, amount, _FlashClient_NationalBank.var.deskNpcId)
                end
            end
        })
        :: continue ::
        ...
        onSelected = function()
            ...
            if (input ~= nil and tonumber(input) ~= nil) then
                ...
                if (amount ~= nil and tonumber(amount) ~= nil) then
                    amount = tonumber(amount)
                    isWaitingForServer = true
                    _FlashLand.toServer("nationalBank:transfer", account.accountId, input, amount, _FlashClient_NationalBank.var.deskNpcId)
                end
            end
        end
    })
