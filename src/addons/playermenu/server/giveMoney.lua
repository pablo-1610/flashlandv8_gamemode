...
_FlashLand.onReceive("playerMenu:giveMoney", function(targetSrc, amount)
    ...
    player:sendData()
    target:sendData()
    player:sendAdvancedNotificationMugShot(targetSrc, _Static_GenericMessages.INTERACTION, (_Static_GenericMessages.PLAYER_GIVEMONEY):format(_FlashUtils.math_price(amount)), "")
    target:sendAdvancedNotificationMugShot(_src, _Static_GenericMessages.INTERACTION, (_Static_GenericMessages.PLAYER_RECEIVEMONEY):format(_FlashUtils.math_price(amount)), "")
end)