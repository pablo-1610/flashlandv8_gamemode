...
_FlashLand.onReceive("staff:moveCoffreOrganisation", function(organisationName)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("staff:moveCoffreOrganisation sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
    ...
    orga:updateCoffreAction(player:getPlayerPos())
    player:serverResponded()
end)