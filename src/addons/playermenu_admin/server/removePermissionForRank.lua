...
_FlashLand.onReceive("staff:removePermissionForRank", function(rankId, permission)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("staff:removePermissionForRank sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
