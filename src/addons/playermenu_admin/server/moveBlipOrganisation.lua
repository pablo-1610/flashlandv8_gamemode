...
_FlashLand.onReceive("staff:moveBlipOrganisation", function(organisationName)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("staff:moveBlipOrganisation sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
    ...
    orga:updateBlip(player:getPlayerPos())
    player:serverResponded()
end)