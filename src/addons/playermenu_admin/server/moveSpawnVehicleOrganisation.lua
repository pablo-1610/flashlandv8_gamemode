...
_FlashLand.onReceive("staff:moveSpawnVehicleOrganisation", function(organisationName)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("staff:moveSpawnVehicleOrganisation sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
    ...
    orga:updateSpawnVehicleAction(player:getPlayerPos())
    player:serverResponded()
end)