...
_FlashLand.onReceive("staff:moveDeleteVehicleOrganisation", function(organisationName)
    local _src = source
    if (not (_FlashServer_Players.exists(_src))) then
        _FlashLand.err(("staff:moveDeleteVehicleOrganisation sans player (%s)"):format(_src))
        return
    end
    ---@type _Player
    ...
    orga:updateDeleteVehicleAction(player:getPlayerPos())
    player:serverResponded()
end)