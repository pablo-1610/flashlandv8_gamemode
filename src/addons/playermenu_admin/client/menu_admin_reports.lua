...
_FlashClient_PlayerMenu.drawer[13] = function(player)
    ...
    if (_FlashLand.countTable(_FlashClient_Staff.getReportList()) <= 0) then
        ...
        for key, data in pairs(_FlashClient_Staff.getReportList()) do
            RageUI.Button(("%sReport de : ~b~%s"):format(_FlashClient_Utils.menu_crossIndicatorIfTrue(not (checkPerm(perm))), data.name), ("Raison : ~b~%s~s~~n~Heure du report : ~b~%s~s~~n~Status : %s"):format(data.reason, data.date, statusReport[data.status]), { RightLabel = "→" }, (checkPerm(perm) and (data.moderatorId == nil) or (data.moderatorId == player.sId)), {
                onSelected = function()
