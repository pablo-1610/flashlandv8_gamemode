...
_FlashLand.onReceive("creator:initMenu", function()
    _FlashClient_Menu.tryOpenMenu(function()
        local selectedComponent, selectedComponentMax, selectedVariation = 0, 0, 0
            ...
            while (true) do
                    ...
                    for _, component in pairs(_ConfigClient.Skin) do
                        RageUI.Button(("Customisation: ~y~%s"):format(component.label), nil, { RightLabel = "→" }, true, {
                            onSelected = function()
                                ...
                                selectedComponentMax = tonumber(_FlashClient_SkinChanger.getMaxValues()[selectedComponent])
                            end
                        }, menus[5])
                    end
                end)
                ...
                RageUI.IsVisible(menus[5], function()
                    for i = 0, selectedComponentMax do
                        RageUI.Button(("Variation n°~y~%s"):format(i), nil, { RightLabel = _FlashClient_Utils.menu_label_selectOrSelected(builder.skin[selectedComponent] == i) }, true, {
                            onSelected = function()
